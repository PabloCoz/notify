            ...
            <span class="inline-flex rounded-md">
                <button type="button" wire:click="resetNotificationCount()"
                    ...
                    {{-- @if (auth()->user()->notification) --}}
                        <span class="ml-2 inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none text-white bg-red-600 rounded-full">{{auth()->user()->notification}}</span>
                   {{--  @endif --}}
                </button>
            </span>
        </x-slot>
